...
namespace WartornNetworking.Client
{
    public class Client
    {
        ...
        public string GetNickName(string clientID) //Yêu cầu server gửi lại cho mình nickname
        {

            Package package = new Package(Messages.Request, Commands.GetNickName,clientID);
            Package reply = SendPackageToServer(package, isGetReply: true);
        ...
        public void SetNickName(string nickname) // Gửi đi nickname
        {
            Package package = new Package(Messages.Request, Commands.SetNickName, nickname);
            SendPackageToServer(package);
